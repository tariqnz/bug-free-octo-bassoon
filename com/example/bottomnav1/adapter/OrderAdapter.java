...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.OrderViewHolder>{
    ...
    public void removeItem(int position) {
        if (mOrderList != null && position >= 0 && position < mOrderList.size()) {
            ...
            notifyItemRemoved(position); // This is sufficient to update the list
        } else {
                        ...
                        public boolean onMenuItemClick(MenuItem menuItem) {
                            ...
                            if(mitem == R.id.viewDet){
                                ...
                                new AlertDialog.Builder(activity) // Use activity context for dialog
                                        ...
                                        .setIcon(android.R.drawable.ic_delete) // Common delete icon
                                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        // User clicked "Yes", proceed with delete
                                        // The 'currentPosition' is already final and captured from the outer scope.
                                        if (currentPosition == RecyclerView.NO_POSITION) {
                                            Toast.makeText(activity, "Error: Could not delete. Item not found.", Toast.LENGTH_SHORT).show();
                                            return;
                                        }
