...
public interface OrderDao {
    ...
    //create by Trae IDE
    @RewriteQueriesToDropUnusedColumns
    @Query("SELECT orders.id, customers.name, customers.father_name, customers.mobile, orders.status, orders.remaining, orders.collectionDate FROM 'orders' LEFT JOIN customers On orders.customerId = customers.Id WHERE status= :InProgress ORDER By orders.collectionDate ASC LIMIT 5")
